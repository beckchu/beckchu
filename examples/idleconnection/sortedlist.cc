...
void EchoServer::onConnection(const TcpConnectionPtr& conn)
{
  ...
  if (conn->connected())
  {
    ...
    node.position = --connectionList_.end();//list::iterator不存在失效的问题
...
void EchoServer::onMessage(const TcpConnectionPtr& conn,//这个应该是heartbeat的写法
                           Buffer* buf,
                           Timestamp time)
{
  ...
  connectionList_.erase(node->position);
  connectionList_.push_back(conn);
  node->position = --connectionList_.end();//算法很简单
...
void EchoServer::onTimer()
{
  ...
  {
    TcpConnectionPtr conn = it->lock();//connection只有remove的时候才会被dtor，mudo没有处理假链接的情况
    if (conn)
    {
